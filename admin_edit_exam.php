...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    $sql_update = "UPDATE dethi SET nganhhoc=?, namhoc=?, hocphan=?, tenfile=?, duongdan=? WHERE id=?";
...
if (isset($_GET['edit_id']) && is_numeric($_GET['edit_id'])) {
    ...
    $sql_edit = "SELECT * FROM dethi WHERE id = ?";
    ...
    if ($result_edit->num_rows == 1) {
        ...
        echo "<p class='error'>Không tìm thấy đề thi cần sửa.</p>";
        exit();
    }
    $stmt->close();
} else {
    echo "<p class='error'>Không xác định đề thi cần sửa.</p>";
    exit();
}
?>
...
<div class="edit-form">
    ...
    <form method="post">
        ...
        <div class="button-container">
            ...
            <a href="admin_quanlidethi.php" class="button">Quay lại trang quản lí</a>
        </div>
    </form>
</div>
