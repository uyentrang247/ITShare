...
if (isset($_GET['delete_id']) && is_numeric($_GET['delete_id'])) {
    ...
    $sql_xoa = "DELETE FROM video WHERE id = ?";
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: admin_quanli_video.php?trang=$trang&xoa=1");
            exit();
        } else {
...
// Đếm tổng số video
$truy_van_dem = "SELECT COUNT(*) AS tong_so FROM video";
...
$truy_van = "SELECT * FROM video ORDER BY id ASC LIMIT ?, ?";
...
<div class="add-button">
    ...
    <a href="admin_quanlidethi.php">Quản lí đề thi</a>
</div>
        ...
        <?php while ($dong = $ket_qua->fetch_assoc()): ?>
            <tr>
                ...
                <td class="action-buttons">
                    <a href="admin_edit_video.php?edit_id=<?= $dong['id'] ?>&trang=<?= $trang_hien_tai ?>">Sửa</a>
